<?php $SubCategory = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $subcategories_table_name WHERE SubCategory_ID ='%d'", intval( $_GET['SubCategory_ID'] ) ) ); ?>
<?php
    if (!isset($_GET['Category_ID'])) { $_GET['Category_ID'] = "";}
	$ID_Matches = $wpdb->get_results($wpdb->prepare("SELECT term_id FROM $wpdb->termmeta  WHERE meta_key='upcp_ID' AND meta_value=%d", intval( $_GET['Category_ID'] ) ) );
	$WC_term_id = 0;
...
?>
		...
		<div class="OptionTab ActiveTab" id="EditSubCategory">
				...
				<?php $Products = $wpdb->get_results( $wpdb->prepare( "SELECT Item_ID, Item_Name FROM $items_table_name WHERE SubCategory_ID='%d'", intval( $_GET['SubCategory_ID'] ) ) );
				...
				<div class="form-wrap SubCategoryDetail">
					...
					<input type="hidden" name="SubCategory_ID" value="<?php echo esc_attr( $SubCategory->SubCategory_ID ); ?>" />
					<input type="hidden" name="WC_term_id" value="<?php echo esc_attr( $SubCategory->SubCategory_WC_ID ); ?>" />
					...
					<div class='form-field'>
						...
						<input name="SubCategory_Name" id="SubCategory_Name" type="text" value="<?php echo esc_attr( $SubCategory->SubCategory_Name );?>" size="60" />
					...
					<div class='form-field'>
						...
						<textarea name="SubCategory_Description" id="SubCategory_Description" rows="5" cols="40"><?php echo esc_html( $SubCategory->SubCategory_Description );?></textarea>
					...
					<div class='form-field'>
						...
						<input id="SubCategory_Image" type="text" size="36" name="SubCategory_Image" value="<?php echo esc_attr( $SubCategory->SubCategory_Image );?>" /> 
