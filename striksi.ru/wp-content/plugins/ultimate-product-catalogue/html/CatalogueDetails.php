...
<?php $Query = $wpdb->prepare("SELECT * FROM $catalogues_table_name WHERE Catalogue_ID ='%d'", intval( $_GET['Catalogue_ID'] ) );
$Catalogue = $wpdb->get_row($Query); ?>
		...
		<div class="OptionTab ActiveTab" id="EditCatalogue">
				<div class="form-wrap CatalogueDetail">
						...
						<input type="hidden" name="Catalogue_ID" value="<?php echo esc_attr( $Catalogue->Catalogue_ID ); ?>" />
						...
						<tr>
								...
								<td><input name="Catalogue_Name" id="Catalogue_Name" type="text" value="<?php echo esc_attr( $Catalogue->Catalogue_Name );?>" size="60" />
	...
	<div id="menu-settings-column" class="metabox-holder">
		...
		<form id="nav-menu-meta" action="admin.php?page=UPCP-options&Action=UPCP_Catalogue_Details&Selected=Catalogue&Catalogue_ID=<?php echo intval( $_GET['Catalogue_ID'] ); ?>#Catalogues" class="nav-menu-meta" method="post" enctype="multipart/form-data">		
...
<!-- Show the products and categories currently in the catalogue, give the user the
	...
	<form id='upcp-catalogue-items-delete' action='admin.php?page=UPCP-options&Action=UPCP_MassDeleteCatalogueItems&Selected=Catalogue&Catalogue_ID=<?php echo intval( $_GET['Catalogue_ID'] ); ?>' method='post'>
	...
	<?php 
		$CatalogueItems = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $catalogue_items_table_name WHERE Catalogue_ID='%d' ORDER BY Position", intval( $_GET['Catalogue_ID'] ) ) );
		foreach ($CatalogueItems as $CatalogueItem) { 
			if ($CatalogueItem->Item_ID != "" and $CatalogueItem->Item_ID != 0) {$CatalogueItemType = "Product"; $CatalogueItemName = $wpdb->get_var( $wpdb->prepare( "SELECT Item_Name from $items_table_name WHERE Item_ID=%d", $CatalogueItem->Item_ID ) );}
			if ($CatalogueItem->Category_ID != "" and $CatalogueItem->Category_ID != 0) {$CatalogueItemType = "Category"; $CatalogueItemName = $wpdb->get_var( $wpdb->prepare( "SELECT Category_Name from $categories_table_name WHERE Category_ID=%d", $CatalogueItem->Category_ID ) );}
		?>
			<tr id="list-item-<?php echo $CatalogueItem->Catalogue_Item_ID; ?>" class="list-item">
				<td class="item-delete"><input type='checkbox' name='Catalogue_Item_ID[]' value='<?php echo esc_attr( $CatalogueItem->Catalogue_Item_ID ); ?>'></td>
