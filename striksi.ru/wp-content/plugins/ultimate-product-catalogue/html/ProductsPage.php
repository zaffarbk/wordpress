...
<?php
	...
	if (empty($per_page) or is_array($per_page) or $per_page < 1 ) {
		$per_page = $screen->get_option('per_page', 'default');
			...
			if (isset($_GET['Page']) and $_GET['DisplayPage'] == "Products") {$Page = intval( $_GET['Page'] );}
			else {$Page = 1;}

			if ( isset( $_GET['OrderBy'] ) and in_array( $_GET['OrderBy'], array( 'Item_Name', 'Item_Description', 'Item_Price', 'Category_Name', 'SubCategory_Name', 'Item_Views' ) ) ) { $OrderBy = $_GET['OrderBy']; }
			else { $OrderBy = 'Item_Date_Created'; }

			$Order = ( isset( $_GET['Order'] ) and $_GET['Order'] == 'DESC' ) ? 'DESC' : 'ASC';

			$Sql = "SELECT * FROM $items_table_name WHERE 1=1 ";
				if (isset($_REQUEST['ItemName'])) {$Sql .= "AND Item_Name LIKE '%" . esc_sql( sanitize_text_field( $_REQUEST['ItemName'] ) ) . "%' ";}
				if (isset($_REQUEST['UPCP_Category_Filter']) and $_REQUEST['UPCP_Category_Filter'] != "All") {$Sql .= "AND Category_ID='" . intval($_REQUEST['UPCP_Category_Filter']) . "' ";}
				if (isset($_REQUEST['UPCP_SubCategory_Filter']) and $_REQUEST['UPCP_SubCategory_Filter'] != "All") {$Sql .= "AND SubCategory_ID='" . intval($_REQUEST['UPCP_SubCategory_Filter']) . "' ";}
				if (isset($_GET['OrderBy']) and $_GET['DisplayPage'] == "Products") {$Sql .= "ORDER BY " . $OrderBy . " ";}
				else {$Sql .= "ORDER BY Item_Date_Created ";}
				if (isset($_GET['OrderBy']) and $_GET['DisplayPage'] == "Products" and $OrderBy == "Item_Price") {$Sql .= "* 1 ";}
				if (isset($_GET['Order'])) {$Sql .= $Order . " ";}

				$TotalProducts = $wpdb->get_results($Sql);
				...
				if (isset($_GET['OrderBy'])) {$Current_Page_With_Order_By .= $Current_Page_With_SubCats . "&OrderBy=" .$OrderBy . "&Order=" . $Order;}
...
<div class="form-field">
	...
	<?php if ($Product->Category_ID != "") {
					  $SubCategories = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $subcategories_table_name WHERE Category_ID=%d ORDER BY SubCategory_Name", $Product->Category_ID ) );
						foreach ($SubCategories as $SubCategory) {
...
<div class="form-field">
    ...
    <?php
    	foreach($TagGroupNames as $TagGroupName){
			...
			$Tags = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $tags_table_name WHERE Tag_Group_ID=%d ORDER BY Tag_Name ASC", $TagGroupID ) );
			if(!empty($Tags)){ ?>
