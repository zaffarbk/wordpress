...
<?php
			if (isset($_GET['Page']) and $_GET['DisplayPage'] == "CustomFields") {$Page = intval( $_GET['Page'] );}
			else {$Page = 1;}
			...
			else { $OrderBy = 'Field_Sidebar_Order'; }

			$Order = ( isset( $_GET['Order'] ) and $_GET['Order'] == 'DESC' ) ? 'DESC' : 'ASC';

			$Sql = "SELECT * FROM $fields_table_name ";
				if (isset($_GET['OrderBy']) and $_GET['DisplayPage'] == "CustomFields") {$Sql .= "ORDER BY " . $OrderBy . " " . $Order . " ";}
