...
<div id='Premium' class='upcp-option-set<?php echo ( $Display_Tab == 'Premium' ? '' : ' upcp-hidden' ); ?>'>
	...
	<table id='premium-options-catalogue-page' class="form-table upcp-options-table<?php echo ( ( $Full_Version != 'Yes' or get_option('UPCP_Trial_Happening') == 'Yes' ) ? ' ewd-upcp-premium-options-table' : '' ); ?>">
		...
		<td>
			...
			<label title='Date' class='ewd-upcp-admin-input-container'><input type='checkbox' name='product_sort[]' value='Date' <?php if(in_array("Date", $Product_Sort)) {echo "checked='checked'";} ?> <?php if ($Full_Version != "Yes") {echo "disabled";} ?>/><span class='ewd-upcp-admin-checkbox'></span> <span><?php _e("Date Added", 'ultimate-product-catalogue')?></span></label><br />
